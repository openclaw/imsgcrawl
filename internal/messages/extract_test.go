...
func TestExtractMessagesPreservesCurrentBodyForMalformedKnownEdit(t *testing.T) {
	ctx := context.Background()
	db, err := sql.Open("sqlite", ":memory:")
	if err != nil {
		t.Fatal(err)
...
insert into message values(1, 'edited-guid', 0, 1, 'iMessage', 0,
'', ?, 2, 0, x'6e6f74206120706c697374');`, makeStreamtypedAttributedBody("known current")); err != nil {
		t.Fatal(err)
	}
	rows, err := extractMessages(ctx, db)
	if err != nil {
	...
	if len(rows) != 1 || rows[0].Text != "known current" || !rows[0].TextAvailable || !rows[0].TextIsCurrent {
		t.Fatalf("malformed edit discarded current body = %#v", rows)
	}
}
