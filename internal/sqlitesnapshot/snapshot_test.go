...
func TestCopyReadsOnlyCommittedWALFrames(t *testing.T) {
	...
	if _, err := writer.Exec(`pragma journal_mode=WAL; pragma wal_autocheckpoint=0; pragma cache_size=1;
create table paired(batch integer, side integer, payload blob);
with recursive n(i) as (select 1 union all select i+1 from n where i<8)
insert into paired select i, side, zeroblob(4096) from n cross join (select 1 side union all select 2);`); err != nil {
		t.Fatal(err)
	}
	...
	defer tx.Rollback()
	if _, err := tx.Exec(`insert into paired select batch+8, 1, payload from paired where side=1`); err != nil {
		t.Fatal(err)
	}
	...
	private, cleanup, err := Copy(ctx, source)
	if err != nil {
		t.Fatal(err)
	}
	defer cleanup()
	...
	if err := reader.DB().QueryRow("select count(*), max(batch) from paired").Scan(&count, &maximum); err != nil {
		t.Fatal(err)
	}
	if count != 16 || maximum != 8 {
		t.Fatalf("uncommitted/mixed generation: count=%d max=%d", count, maximum)
	}
	for suffix, want := range before {
		got, err := os.ReadFile(source + suffix)
		if err != nil || !bytes.Equal(got, want) {
			t.Fatalf("live source changed for %q: %v", suffix, err)
		}
	}
}

func TestCopyRecoversRollbackJournalOnlyInPrivateDirectory(t *testing.T) {
	ctx := context.Background()
	source := filepath.Join(t.TempDir(), "source.db")
	writer, err := sql.Open("sqlite", source)
	if err != nil {
...
func TestCopyPreservesCancellationDuringFinalVerification(t *testing.T) {
	source := filepath.Join(t.TempDir(), "source.db")
	...
	defer cancel()
	attempts := 0
	_, cleanup, err := copyWithVerifier(ctx, source, func(context.Context, string) error {
		attempts++
		if attempts == 5 {
			cancel()
			return ctx.Err()
		}
		return errors.New("synthetic verification retry")
	})
	cleanup()
